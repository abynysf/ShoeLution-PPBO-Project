...
public class TempPemesananDao implements TempPemesananService{
    ...
    public void updateData(TempPemesananModel tmp_pModel) {
        PreparedStatement st = null;
        String sql = "UPDATE Temp_Pemesanan SET nama_sepatu=?, harga=?, jml_pemesanan=?, subTotal_Pemesanan=? WHERE id_sepatu=?";
        try{
            st = connection.prepareStatement(sql);
            
            st.setString(1, tmp_pModel.getsModel().getNamaSepatu());
            st.setDouble(2, tmp_pModel.getsModel().getHarga());
            st.setInt(3, tmp_pModel.getDpModel().getJmlPemesanan());
            st.setDouble(4, tmp_pModel.getDpModel().getSubtotalPemesanan());
            st.setString(5, tmp_pModel.getsModel().getIdSepatu());
    ...
    public void deleteData(TempPemesananModel tmp_pModel) {
        PreparedStatement st = null;
        String sql = "DELETE FROM Temp_Pemesanan WHERE id_sepatu=?";
        try{
            st = connection.prepareStatement(sql);
